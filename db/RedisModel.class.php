...
class RedisModel {
    ...
    private function __construct($db = null) {
        ...
        $this->redis->connect($cfg['db_host'], $cfg['db_port']);
        if(isset($cfg['db_pwd']))$this->redis->auth($cfg['db_pwd']);
        return $this->redis;
    }

    private function __clone(){}

    static public function instance($db = null){
        if(!isset(self::$instance)){
            self::$instance = new RedisModel($db);
     ...
     * 队列右边安全取出
     * @param $key 要取的key
     * @param $key_bak 备份的key
     * @param $time 取不到值时的阻塞时间
     * @return string
     */
    public function brpoplpushs($key, $key_bak, $time)
    {
        return $this->redis->brpoplpush($key,$key_bak,$time);
    }
     ...
     * @return int
     */
    public function lpushs($key, $data)
    {
     ...
     * @return int
     */
    public function lrems($key, $data)
    {
        return $this->redis->lrem($key, $data);
    }
