...
//               ->withWhiteListHosts($this->ip)
               ->withCredentials($this->db_user, $this->db_pwd)
                ->withDefaultTimeout(3)
                ->withConnectTimeout(3)
                ->withRequestTimeout(3)
   ...
   private function __clone(){}
    ...
    static public function instance($config = []){
        if(!isset(self::$instance)){
            self::$instance = new CassandraDb($config);
    ...
    static public function clean_instance(){
        self::$instance = null;
    }
